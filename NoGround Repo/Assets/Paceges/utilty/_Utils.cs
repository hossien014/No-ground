...
namespace Abed.Utils
{
      public static class _Utils
      {
            ...
            #endregion
            public static void DeleteOBjectWhitChild(GameObject obj, bool itsInEditor)
            {
                  foreach (Transform ChildObj in obj.transform)
                  {
                        if (itsInEditor) GameObject.Destroy(ChildObj);
                        else GameObject.Destroy(ChildObj);
                  }
            }
            public static void ShowBoxCastRay2d(BoxCollider2D colider, float Y_offset, float X_offset)
            {
                  Debug.DrawRay(colider.bounds.center + new Vector3(colider.bounds.extents.x + X_offset / 2, 0), Vector3.down * (colider.bounds.extents.y + Y_offset), Color.green); //right
                  Debug.DrawRay(colider.bounds.center - new Vector3(colider.bounds.extents.x + X_offset / 2, 0), Vector3.down * (colider.bounds.extents.y + Y_offset), Color.green); // left 
                  Debug.DrawRay(colider.bounds.center - new Vector3(colider.bounds.extents.x + X_offset / 2, colider.bounds.extents.y + Y_offset), Vector3.right * (colider.bounds.extents.x * 2 + X_offset), Color.green); //down line
                  Debug.DrawRay(colider.bounds.center - new Vector3(colider.bounds.extents.x + X_offset / 2, -colider.bounds.extents.y), Vector3.right * (colider.bounds.extents.x * 2 + X_offset), Color.green); //top line

            }
            public static Vector3 Worldpos2D(Camera m_camera)
            {
                  return m_camera.ScreenToWorldPoint(Input.mousePosition);
            }
            public static Vector3 Worldpos2D()
            {
                  return Camera.main.ScreenToWorldPoint(Input.mousePosition);
            }
            ///  <summary>
